...
def Btn2_Click():
	...
	try:
		...
		match = re.search(EditValue, res_word[0])
		if match:
		    res_word = res_word[0]
		else:
			res_word = "入力された単語はwikipedia内に存在しませんでした。"
	catch:
		res_word = "入力された単語はwikipedia内に存在しませんでした。"

	txtbox = tkinter.Text(sub_win2, width=200, height=50)
	txtbox.insert("1.0", res_word)
	txtbox.pack()
	"""
...
def Btn4_Click():
	...
	sub_win3.title('文法的におかしいかもしれません。')
...
label2 = ttk.Label(text = 'ニュースを調べて面接時の質問に答えられるようにしましょう。(30秒程かかります)', font = ('メイリオ', 15))
